...
public class ExcelUtility {
    public static void logScenarioToExcel(Scenario scenario, LocalTime start, LocalTime end, Duration duration, String browserName) {
        ...
        if (!file.exists()) {
            ...
            row1.createCell(5).setCellValue(duration.toMillis() + " milliseconds");
            row1.createCell(6).setCellValue(scenario.getStatus().toString());
            try {
            ...
            try {
                ...
                row.createCell(6).setCellValue(String.valueOf(scenario.getStatus()));
                FileOutputStream fileOutputStream = new FileOutputStream(path);
                workbook.write(fileOutputStream);
                workbook.close();
                fileOutputStream.close();
